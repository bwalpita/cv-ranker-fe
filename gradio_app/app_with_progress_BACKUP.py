...
def call_ranking_api(payload: Dict[str, Any], progress=gr.Progress()) -> Dict[str, Any]:
    ...
    try:
        ...
        # Simulate gradual progress updates (API is synchronous)
        # We'll update progress based on elapsed time
        timeout = 300  # 5 minutes max
        start_time = time.time()
        
        # Send request
        progress(10, desc="Sending request to ranking engine...")
        response = requests.post(API_URL, json=payload, timeout=timeout)
        
        if response.status_code == 200:
            progress(100, desc="✅ Ranking complete!")
...
"""
        ...
        # Add SHAP explanation with better formatting
        ...
        if shap.get('explanation'):
            ...
            # Show top factors in a more readable format
            explanation_text = shap.get('explanation', '')
            if 'Top Contributing Factors:' in explanation_text:
                # Extract and format the top factors
                factors_section = explanation_text.split('Top Contributing Factors:')[1] if 'Top Contributing Factors:' in explanation_text else explanation_text
                
                # Limit to top 5 factors for readability
                baseline_display += "**Top 5 Key Factors:**\n"
                baseline_display += "```\n"
                baseline_display += factors_section[:500]  # Show more than 300 chars
                baseline_display += "\n```\n"
                baseline_display += "\n💡 *Tip: Positive factors (↑) increased the score, negative factors (↓) decreased it. Larger numbers = stronger impact.*\n"
            else:
                baseline_display += f"{explanation_text[:400]}...\n"
...
def format_flowise_output(response: Dict[str, Any], payload: Dict[str, Any] = None) -> str:
    ...
    try:
            ...
            try:
                ...
                if isinstance(raw_resp, dict) and 'usedTools' in raw_resp:
                    for tool in raw_resp['usedTools']:
                        if tool.get('tool') == 'linkedin_behavior_scorer':
                            try:
                                output = json.loads(tool.get('toolOutput', '{}'))
                                linkedin_breakdown = output
                            except:
                                pass
                        elif tool.get('tool') == 'facebook_behavior_scorer':
                            try:
                                output = json.loads(tool.get('toolOutput', '{}'))
                                facebook_breakdown = output
                            except:
                                pass
            except:
                pass
        ...
        if linkedin_breakdown:
            ...
            linkedin_breakdown_html = f"""
                <div class="section">
                    ...
                    <h3 style="margin-top: 25px; margin-bottom: 15px; color: #1a202c;">📝 Individual Post Scores (LinkedIn)</h3>
        ...
        if facebook_breakdown:
            ...
            facebook_breakdown_html = f"""
                <div class="section">
                    ...
                    <h3 style="margin-top: 25px; margin-bottom: 15px; color: #1a202c;">📝 Individual Post Scores (Facebook)</h3>
                    <div style="max-height: 600px; overflow-y: auto; border: 1px solid var(--border-color); border-radius: 8px; padding: 15px; background-color: var(--bg-secondary);">
            """
        ...
        html = template.format(
            ...
            match_score=match_score,
            linkedin_score=linkedin_score*100,
            linkedin_label=linkedin_label,
            facebook_score=facebook_score*100,
        ...
        return f"""
        <div style="background-color: #fff5f5; padding: 20px; border-radius: 8px; color: #742a2a; font-family: sans-serif;">
            <h3>⚠️ Error Displaying Results</h3>
            <p><strong>Error:</strong> {str(e)}</p>
            <p style="font-size: 0.9em; margin-top: 10px; color: #999;">Please try refreshing the page or contacting support.</p>
        </div>
        """
...
def load_search_history() -> str:
    ...
    try:
        ...
        for i, record in enumerate(history, 1):
            ...
            display += '<div style="border: 1px solid #ddd; padding: 12px; margin: 10px 0; border-radius: 8px; background-color: #f9f9f9;">'
            display += '<div style="display: flex; justify-content: space-between; align-items: start;">'
            display += '<div style="flex-grow: 1;">'
            display += f'<h3 style="margin: 0 0 8px 0; color: #2c3e50;">{i}. {candidate} - {job_title}</h3>'
            display += f'<p style="margin: 4px 0; font-size: 0.9em; color: #555;">📅 <strong>Time:</strong> {timestamp}</p>'
            display += f'<p style="margin: 4px 0; font-size: 0.9em; color: #555;">📊 <strong>Match Score:</strong> {score_pct}</p>'
            display += f'<p style="margin: 4px 0; font-size: 0.9em; color: #555;">💬 <strong>Recommendation:</strong> {recommendation}...</p>'
            display += f'<p style="margin: 4px 0; font-size: 0.85em; color: #888;">🔑 ID: {record_id}</p>'
            display += '</div>'
            # Use data attribute instead of complex onclick
            display += f'<button class="delete-record-btn" data-record-id="{record_id}" style="background-color: #e74c3c; color: white; border: none; padding: 8px 12px; border-radius: 4px; cursor: pointer; font-size: 0.9em; margin-left: 10px; white-space: nowrap;">🗑️ Delete</button>'
            display += '</div></div>'
...
""")
                        flowise_output = gr.HTML(value="<p style='color: #888;'>Flowise response will appear here once integrated...</p>")
